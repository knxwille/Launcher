...
std::set<std::wstring> g_HiddenApps;
...
const char* GetT(const std::string& key) {
    ...
    if (key == "ADD_APP") return "Uygulama Ekle";
    if (key == "ALREADY_EXISTS_MSG") return "Bu uygulama zaten listede ekli!";
    if (key == "WARNING") return "Uyari";
    if (key == "HIDE_APP") return "Uygulamayi Gizle";
    return "??";
}
...
std::wstring SelectFile(const std::wstring& defaultFolder = L"") {
    ...
    if (SUCCEEDED(CoCreateInstance(CLSID_FileOpenDialog, NULL, CLSCTX_ALL, IID_IFileOpenDialog, reinterpret_cast<void**>(&pFileOpen)))) {
        ...
        COMDLG_FILTERSPEC rgSpec[] = { { L"Executable Files", L"*.exe" } };
...
void SaveSettings() {
    ...
    if (file.is_open()) {
        ...
        file << "\n[Hidden]\n";
        for (const auto& path : g_HiddenApps) {
            file << "Path=" << WStringToString(path) << "\n";
        }
        file.close();
    }
}

bool IsAppAlreadyAdded(const std::wstring& path) {
    std::wstring standardPath = path;
    std::transform(standardPath.begin(), standardPath.end(), standardPath.begin(), ::towlower);
    for (auto& ch : standardPath) if (ch == L'/') ch = L'\\';

    for (const auto& app : g_Apps) {
        std::wstring existingPath = app.exePath;
        std::transform(existingPath.begin(), existingPath.end(), existingPath.begin(), ::towlower);
        for (auto& ch : existingPath) if (ch == L'/') ch = L'\\';

        if (existingPath == standardPath) return true;
    }
    return false;
}

void SortAppsAlphabetically() {
    std::sort(g_Apps.begin(), g_Apps.end(), [](const AppInfo& a, const AppInfo& b) {
    ...
    try {
        for (const auto& entry : fs::directory_iterator(g_PortableDir)) {
            if (entry.is_directory()) {
                if (fs::exists(entry.path() / ".hidden") || g_HiddenApps.count(entry.path().wstring())) continue;

                for (const auto& file : fs::directory_iterator(entry.path())) {
                    if (file.path().extension() == ".exe") {
                        if (IsAppAlreadyAdded(file.path().wstring())) continue;
...
void AddManualApp() {
    std::wstring exePath = SelectFile(g_PortableDir);
    if (exePath.empty()) return;

    if (IsAppAlreadyAdded(exePath)) {
        MessageBoxW(NULL, StringToWString(GetT("ALREADY_EXISTS_MSG")).c_str(), StringToWString(GetT("WARNING")).c_str(), MB_OK | MB_ICONWARNING);
        return;
    }

    AppInfo info;
    info.exePath = exePath;
    info.name = fs::path(exePath).stem().string();
    info.iconPath = "icons/" + info.name + ".png";
    if (!fs::exists("icons")) fs::create_directory("icons");
    if (ExtractAndSaveIcon(info.exePath, info.iconPath)) {
    ...
    while (std::getline(file, line)) {
        ...
        if (section == "[Settings]") {
            ...
            if (eqPos != std::string::npos && pipePos != std::string::npos) {
                ...
                if (!IsAppAlreadyAdded(info.exePath)) {
            ...
            if (line.find("Path=") == 0) {
                g_HiddenApps.insert(StringToWString(line.substr(5)));
            }
        }
    }
    file.close();
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nShowCmd) {
    ...
    fs::current_path(fs::path(exeBuffer).parent_path());
    (void)CoInitializeEx(NULL, COINIT_APARTMENTTHREADED);

    if (!glfwInit()) return 1;
    glfwWindowHint(GLFW_RESIZABLE, GLFW_TRUE);
    glfwWindowHint(GLFW_VISIBLE, GLFW_FALSE);
    GLFWwindow* window = glfwCreateWindow(500, 700, "Loading...", NULL, NULL);
    if (!window) return 1;

    glfwMakeContextCurrent(window);
    LoadSettings();
    glfwSetWindowTitle(window, GetT("TITLE"));
    ...
    else if (g_Apps.empty()) ScanApps();

    while (!glfwWindowShouldClose(window)) {
        glfwPollEvents();
        if (!glfwGetWindowAttrib(window, GLFW_VISIBLE)) { Sleep(10); continue; }
        ...
        if (ImGui::Combo("##Lang", &currentLangIdx, langFiles, IM_ARRAYSIZE(langFiles))) {
            ...
            glfwSetWindowTitle(window, GetT("TITLE"));
            wcscpy_s(g_NotifyIconData.szTip, StringToWString(GetT("TRAY_TIP")).c_str());
            Shell_NotifyIconW(NIM_MODIFY, &g_NotifyIconData);
            SaveSettings();
        }
        ImGui::PopItemWidth();
        ImGui::SameLine();
        if (ImGui::Button(GetT("CHANGE_DIR"))) {
            ...
            if (!newDir.empty()) {
                g_PortableDir = newDir;
                g_Apps.clear();
                ScanApps();
            }
        }
        ImGui::SameLine();
        if (ImGui::Button(GetT("REFRESH"))) ScanApps();
        ImGui::SameLine();
        if (ImGui::Button(GetT("ADD_APP"))) AddManualApp();

        ImGui::TextColored(ImVec4(0.4f, 0.4f, 0.4f, 1.0f), "%s: %s", GetT("DIR_LABEL"), WStringToString(g_PortableDir).c_str());
        ...
        for (int i = 0; i < (int)g_Apps.size(); i++) {
            ...
            if (ImGui::BeginPopupContextItem("ItemContextMenu")) {
                if (ImGui::MenuItem(GetT("HIDE_APP"))) {
                    g_HiddenApps.insert(fs::path(app.exePath).parent_path().wstring());
                ...
                if (ImGui::MenuItem(GetT("EDIT_PATH"))) {
                    ...
                    if (!newExe.empty()) {
                        ...
                        app.exePath = newExe;
                        app.name = fs::path(newExe).stem().string(); // İsmi dosya ismine göre güncelle

                        // İkon dosyasını yeni isme göre ayarla ve yenile
                        std::string oldIcon = app.iconPath;
                        app.iconPath = "icons/" + app.name + ".png";
